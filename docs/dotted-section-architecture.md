# Dotted Section Architecture

This document serves as a blueprint and code snapshot for the `DottedSection` component (`components/dotted-section.tsx`). It details the specific scroll behavior, layered sticky architecture, and focal lens opacity logic so that the component can be easily restored if changes cause regressions.

## The Architecture (3 Layers)

The section uses a specific, battle-tested CSS stacking architecture to achieve native-feeling scrollytelling without layout conflicts or scroll jank.

1. **Layer 1 (Background):** A sticky `DottedSurface` WebGL background. 
   - `z-index: 0`
   - Uses `marginBottom: '-100vh'` to allow the rest of the page to flow over it naturally while it stays pinned.
2. **Layer 2 (Headline):** A sticky intro heading.
   - `z-[10]`
   - Driven by Framer Motion's `useScroll` and `useSpring`. It smoothly translates up and fades out over the first 30% of the section's scroll.
   - Importantly, it has `pointer-events-none` so it does not block mouse interactions for the content scrolling underneath it.
3. **Layer 3 (Industry List):** A naturally scrolling list of items.
   - `z-[20]`
   - Driven by a native `requestAnimationFrame` loop that calculates the distance of each item to the center of the viewport (Focal Lens effect).
   - Because it naturally scrolls, it does not require complex `translateY` tracking to move up the screen, preventing "jumping" behavior.

## Focal Lens Logic

Instead of fading everything in at a specific scroll percentage (which causes elements to "jump" onto the screen), the industry items calculate their opacity based purely on their distance from the vertical center of the screen (`viewportH / 2`).

- **Below Center (Rising Up):** Items start at a minimum opacity of `0.35` (so they are visible) and have a `40px` downward shift. As they approach the center, the shift reaches `0` and opacity reaches `1.0`.
- **At Center (Hold):** Within an `80px` threshold of the center (`fullColorZone`), items are at `1.0` opacity.
- **Above Center (Fading Out):** As items pass the center, they gently fade out to `0.05` opacity.
